...
/**
 * This class holds everything related to a balls used in this app. Arrays of balls
 * to be drawn on a screen - scattered and destination balls, selected ball (the one
 * that user to click on) and a dummy ball.
 *
 * @author v.yanchenko
 */
public class Balls {
    ...
    //** Dummy ball.
    private BallMoved ballDummy;
    //** This is a field that holds a destination balls.
    private Ball[] ballsDestination;
    ...
    public Balls(BallsLogic ballsLogic, BallsImages ballsImages, int screenWidth, int screenHeight, int number) {
         ...
         * be seen.
         */
        ballDummy = new BallMoved(screenWidth, screenHeight, false);
        ...
        for (int i = 0; i < number; i++) {
            ...
            ballsDestination[i] = new Ball(screenWidth,
                    screenHeight, false);
            //** Instantiating a scattered balls
            ballsScattered[i] = new BallMoved(screenWidth, screenHeight,
                    true);
...
//                }
    ...
    /**
     * Computing data for balls - deltas to move on, distances among balls.
     * @param doComputations
     */
    public void computeMetaData(boolean doComputations) {

        double maxDistance = 0;

        //** Make a computations, in case it is allowed )
        if (doComputations) {

            for (int i = 0; i < ballsScattered.length; i++) {

                //** computing distance between the dots
                ballsMetaData[i].setDistance(
                        Geometry.computeDistance(
                        ballsScattered[i].getX(),
                        ballsScattered[i].getY(),
                        ballsDestination[i].getX(),
                        ballsDestination[i].getY()));
                if (BallsMetaData.convergeAtOnce) {
                    if (maxDistance < ballsMetaData[i].getDistance()) {
                        maxDistance = ballsMetaData[i].getDistance();
                    }
                }
                //** computing delta for x
               ballsScattered[i].setdX(
                        (ballsDestination[i].getX()
                        - ballsScattered[i].getX())
                        / ballsMetaData[i].getDistance());
                //** computing delta for y
                ballsScattered[i].setdY(
                        (ballsDestination[i].getY()
                        - ballsScattered[i].getY())
                        / ballsMetaData[i].getDistance());
            }
            if (BallsMetaData.convergeAtOnce) {
                ...
                for (i = 0; i < ballsMetaData.length; i++) {
                   ...
                   ballsScattered[i].setdX(ballsScattered[i].getdX() / distanceRatio);
                    ballsScattered[i].setdY(ballsScattered[i].getdY() / distanceRatio);
                }
            }
        }
    }
    ...
    public BallMoved[] randomizeBalls(BallMoved[] balls,
                                      int screenWidth, int screenHeight) {
        for (BallMoved ball : balls) {
            ball.setX(Math.random() * screenWidth);
            ball.setY(Math.random() * screenHeight);
        }
        return balls;
    }
...
//    }
    ...
    public BallMoved getBallSelected() {
        return ballSelected;
    }

    public void setBallSelected(BallMoved ballSelected) {
        this.ballSelected = ballSelected;
    }

    public Ball getBallDummy() {
        return ballDummy;
    }

    public void setBallDummy(BallMoved ballDummy) {
        this.ballDummy = ballDummy;
    }

    public Ball[] getBallsDestination() {
        return ballsDestination;
    }

    public void setBallsDestination(Ball[] ballsDestination) {
        this.ballsDestination = ballsDestination;
    }

    public BallMoved[] getBallsScattered() {
        return ballsScattered;
    }

    public void setBallsScattered(BallMoved[] ballsScattered) {
